...
async function checkConfig(rootDir) {
  const cfgPath = process.env.AIDR_CONFIG || path.join(rootDir, 'config.json');
...
async function checkState(rootDir) {
  const statePath = process.env.AIDR_STATE || path.join(rootDir, 'state.json');
...
async function runDoctor(opts = {}) {
  ...
  // Must resolve the same path checkConfig validates, or the SMTP/capsolver
  // checks would silently run against {} when AIDR_CONFIG is set.
  let cfg = {};
  try {
    const cfgPath = process.env.AIDR_CONFIG || path.join(rootDir, 'config.json');
    cfg = JSON.parse(fs.readFileSync(cfgPath, 'utf8'));
