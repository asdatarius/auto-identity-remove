...
// History is a persistent WRITE target (unlike the read-only data/ files shipped
// with the app), so in a container it must live on the mounted data dir — the
// app dir is root-owned and wiped on image upgrade.
const DATA_DIR     = process.env.AIDR_DATA || path.join(__dirname, '..', 'data');
...
function appendManyToHistory(entries) {
  ...
  // Best-effort: an unwritable history dir (e.g. read-only/foreign-owned mount)
  // must not kill the run — the scan results were already reported.
  try {
    ...
    console.warn(`⚠️  serp-scan: could not write history to ${HISTORY_PATH}: ${err.message}`);
  }
}
