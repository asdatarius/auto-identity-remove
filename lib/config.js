...
// Paths are env-overridable so a container can point config + state at a single
// mounted data dir (e.g. /data). Defaults are unchanged for native use.
// AIDR_STATE_PATH is upstream's name for the state override — accept both.
const CONFIG_PATH     = process.env.AIDR_CONFIG || path.join(__dirname, '..', 'config.json');
const CONFIG_ENC_PATH = CONFIG_PATH + '.enc';
const _stateOverride  = process.env.AIDR_STATE || process.env.AIDR_STATE_PATH;
const STATE_PATH      = _stateOverride
  ? path.resolve(_stateOverride)
